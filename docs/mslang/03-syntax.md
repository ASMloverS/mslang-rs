...
常量必须在声明时初始化，之后不可修改。常量值必须是**编译期可求值的常量表达式**，允许的语法形式：

- 字面量（整数、浮点、字符串、布尔、nil）
- 其他常量引用
- 一元取反（`-`、`~`）作用于上述形式
- 二元算术运算（`+` `-` `*` `/` `//` `%` `**` `&` `|` `^` `<<` `>>`）作用于上述形式
- 括号分组

**不允许**出现在 `const` 右侧的：函数调用、变量引用（非 const）、方法调用、比较/逻辑运算、集合构造。
...
### 多目标赋值语义：右侧表达式先全部求值，构造元组，再按位置解包到左侧各目标。**解包数量必须匹配**：右侧元素数量与左侧目标数量不一致时抛出 `ValueError`。
...
`for` 循环变量数量必须与每次迭代产出的值数量匹配。当使用两个变量时（`for k, v in ...`），迭代器每次须产出恰好包含两个元素的序列（如 `dict.items()` 返回的元组对），否则抛出 `ValueError`。
...
- 多个 defer 按**后进先出**执行
- defer 异常交互规则详见 [05-control-flow](05-control-flow.md#defer-异常交互规则)
...
注意：`{}` 是空 dict，空 set 用 `set()`。单元素花括号 `{1}` 不匹配 set 语法，将被解析为分组表达式（等价于 `1`）。单元素 set 使用 `{1,}`（尾部逗号）或 `set([1])`。编译器会对 `{single_value}` 形式的花括号表达式发出警告，提示可能意图构造 set。
