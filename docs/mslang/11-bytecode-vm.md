...
跳转偏移量为有符号 16 位整数，相对于当前指令位置。`JUMP` 用于 if/else 等前向分支，`JUMP_BACK` 用于 while/for 循环回边。**限制**：单次跳转最大距离 ±32KB；超大函数需拆分为多个子函数。
...
| `RETURN` | — | 从函数返回 |
| `TAIL_CALL` | `argc(1)` | 尾调用（预留，暂未启用优化） |
...
| `SET_ADD` | `slot(1)` | 弹出栈顶值，加入 slot 处的 set 局部变量（集合推导式） |
| `DICT_INSERT` | `slot(1)` | 先弹 val、再弹 key，插入 slot 处的 dict 局部变量（字典推导式） |
