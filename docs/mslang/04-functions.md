...
> **警告**：可变对象作为默认值时，所有调用共享同一对象。`fn foo(items = []) { items.push(1); return items }` 每次调用返回同一个列表。推荐使用 `fn foo(items = nil) { if items == nil { items = [] }; ... }` 模式。
...
> **限制**：MVP 阶段不支持关键字参数调用（`fn(key=val)`）和 `**kwargs` 收集。仅支持位置参数传递。后续版本考虑增加。
...
**解包规则**：
- 右侧值必须是可迭代对象，否则抛出 `TypeError`
- 左侧目标数量与右侧元素数量不一致时抛出 `ValueError`
- 单值解包：`a = fn()` 返回非元组值时，`a` 直接获得该值（无需特殊处理）
- 多值解包：`a, b = fn()` 时，`fn()` 的返回值必须恰好包含 2 个元素的序列
...
匿名函数是完全功能的闭包，可以有任意复杂的函数体。匿名函数支持默认参数和可变参数，语法与命名函数一致：`fn(x, y = 10) { return x + y }`。
...
尾调用优化暂不实现（`TAIL_CALL` 操作码已预留，后续版本考虑启用）。
