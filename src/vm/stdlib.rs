...
use super::object::{
    alloc_dict, alloc_exception, alloc_file_handle, alloc_future, alloc_list, alloc_module,
    alloc_set, alloc_string, alloc_tuple, read_dict, read_file_handle, read_file_handle_mut,
    read_list, read_module_mut, read_set, read_str, CmpOp, DictMap, FutureState, MsObjHeader,
    Object, TypeTag,
};
use super::VM;
...
fn gc_set_threshold(vm: &mut VM, args: &[Object]) -> Result<Object, String> {
    ...
    match kind.as_str() {
        ...
        "minor" => {
            ...
            vm.heap.next_minor_gc = (mb as usize) * 1024 * 1024;
            Ok(Object::Nil)
        }
...
fn gc_stats(vm: &mut VM, _args: &[Object]) -> Result<Object, String> {
    ...
    // MVP STW 单线程：固定返回 1，忽略用户 set_gc_threads 设置。
    let gc_threads = 1i64;
...
"#;
    ...
    fn test_gc_module_registration() {
        ...
        unsafe {
            ...
            for name in [
                ...
                "set_threshold", "set_promotion_age", "set_gc_threads", "stats",
                "count", "mem_alloc", "mem_live",
            ] {
                assert!(m.exports.contains_key(name), "missing export: {}", name);
            }
        }
    ...
    fn test_gc_stats_returns_all_fields() {
        ...
        assert!(dict_int(&result, "young_size") >= 0);
        ...
        assert_eq!(dict_int(&result, "gc_threads"), 1); // MVP STW 固定 1
    ...
    fn test_gc_set_threshold_minor() {
        ...
        assert_eq!(v.heap.next_minor_gc, 8 * 1024 * 1024);
    }
    ...
    fn test_gc_set_gc_threads_accepts_value() {
        let mut v = vm();
        gc_set_gc_threads(&mut v, &[Object::Int(4)]).unwrap();
        assert_eq!(v.heap.gc_threads_setting, 4);
        // stats 仍返回 1（MVP STW）。
        let _ = gc_collect(&mut v, &[]);
        let result = gc_stats(&mut v, &[]).unwrap();
